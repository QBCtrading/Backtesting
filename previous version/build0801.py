...
def plotForwardCurve(calc_date,yield_curve):
    print('yield_curve dates:',yield_curve.dates()[0])
    l = []
    for i in range(0,40):
        l.append(yield_curve.forwardRate(i/4,i/4+0.25,ql.Compounded).rate())
    ts = pd.Series(l)
    ts.index = np.arange(0,10,0.25)
    plt.subplot(211)
    plt.plot(ts,drawstyle='steps-pre')
    
    l = []
    for i in range(1,41):
        l.append(createSwap(calc_date, str(3*i)+'M', china_index, swap_engine).fairRate())
    ts = pd.Series(l)
    ts.index = np.arange(0.25,10.25,0.25)
    plt.subplot(212)
    plt.plot(ts,drawstyle='steps-pre')
    plt.subplot(211)
